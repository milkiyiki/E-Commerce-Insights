...
# Info hasil filter
st.write(f"🔎 Menampilkan {len(filtered_df)} data setelah difilter.")

if filtered_df.empty:
    st.warning("Data tidak ditemukan untuk filter yang dipilih. Silakan ubah filter.")
    st.stop()
...
fig = px.bar(seasonality, x="month", y="avg_order_count", title="Rata-rata Pesanan per Bulan",
             labels={"month": "Bulan", "avg_order_count": "Rata-rata Jumlah Pesanan"},
             color="avg_order_count", text_auto=True)
...
# Scatter plot
fig = px.scatter(filtered_df, x="review_score", y="payment_value",
                 title="Hubungan antara Rating dan Nilai Pembayaran",
...
wordcloud = WordCloud(width=800, height=400, background_color="white").generate(
    " ".join(filtered_df["product_category_name"].dropna())
)
